   ...
   // test.use({ storageState: authFile });
    ...
    test('Verify all products are displayed on the inventory page', async ({ inventoryPage, inventoryData }) => {
        await test.step('Verify all inventory items are visible', async () => {
            expect(inventoryPage.inventoryItems).toHaveCount(inventoryData.listNumber);
        });
    });
...
test('Logout from the inventory page', async ({ inventoryPage, page, loginPage, loginData }) => {
...
test.describe('Inventory page - not loggin', () => {
    ...
    test('Accessing the inventory page without login', async ({ inventoryPage, page, inventoryData }) => {
        ...
        await test.step('Verify the user is redirected to the login experience', async () => {
            await expect(inventoryPage.usernameLabel).toBeVisible();
        });
    });
});
